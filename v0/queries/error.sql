with 
   ...
   ,
    measurements as ( select 
        school, year, class, exam, date, stream, student, subject, `measurement`, 
        'value' as measurement, 
        value as value 
    from grading union all select 
        `school`, `year`, `class`, `exam`, `date`, `stream`, `student`, `subject`, `measurement`, 
...
crumb as (
                select
                    crown.*,
                    json_object(
                        'id', concat_ws('/', measurement),
                        'value',value
                    ) as value2
                 from crown    
            ),
 crest as (select school,year,class,exam,date,stream,student,subject, json_arrayagg(value2) as raw_values from crumb group by school,year,class,exam,date,stream,student,subject) 
/*select * from crest limit 40 offset 0*/
