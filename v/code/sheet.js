...
class panel extends view {
    ...
    attributes) {
        ...
        switch (type.type) {
            ...
            case 'id':
                this.proxy = this.get_element(type.id);
                break;
            //
    ...
    set_attributes(attr) {
        ...
        if (attr.textContent !== undefined)
            this.proxy.textContent = attr.textContent;
        if (attr.className !== undefined)
            this.proxy.classList.add(attr.className);
        ...
        if (attr.io !== undefined)
            io.io.create_io({ element: this.proxy, page: this.sheet }, attr.io);
        //
        //Set the id to that of the proxy
        if (attr.id !== undefined)
            this.proxy.id = attr.id;
    }
    //Get ready to fix the left and right margin panels by availing the
    ...
    get col_index() {
        ...
        //visually
        if (this.proxy instanceof HTMLTableCellElement)
            return this.proxy.cellIndex;
        //
...
class matrix extends panel {
    ...
    get_cell(id, tr) {
        ...
        const panel = this.header.pivot_crumbs.get(id);
        //
        //The identified panel must be defined
        if (!panel)
            throw new schema.mutall_error(`No panel matches cell id string '${id}' among the header crumb panels in the pivot`);
        //
        //Retrieve the colum index
        const column_index = panel.col_index;
        //
        //Get the cell that matches the id
        const cell = tr.cells[column_index];
        //
        return cell;
    }
}
class header extends panel {
    ...
    show_crests(type) {
        ...
        //They have text labels that match their ids.
            ...
            for (let index = 0; index < this.factors.crest.length; index++) {
                //
...
class body extends panel {
    ...
    create_empty_row(row) {
        ...
        const tr = this.create_element('tr', this.proxy);
        //
        ...
        if (pivots.length === 0)
            ...
            //NB. cell_id is a Map
            new panel({ type: 'create', tagname: 'td', parent: tr }, this, { io: this.get_body_io_type(new Map(), row) });
        else
            //
            ...
            pivots.forEach(cell_id => new panel({ type: 'create', tagname: 'td', parent: tr }, this, { io: this.get_body_io_type(cell_id, row) }));
        //
    ...
    populate_row(data_row, row_index) {
        ...
        const tr = this.proxy.rows[row_index];
        //
    ...
    populate_cell(id, value, tr) {
        ...
        //given one
        const col_index = this.matrix.header.cell_indices.get(id);
        //
    ...
    */
    data_row) {
        ...
        const td = this.matrix.get_cell(id, tr);
        //
        //Use the td to get the io that is associated with that it
        const Io = io.io.get_io(td);
        //
    ...
    get_cell_id(id) {
        //
        //Get the id in the header pivots map
        const panel = this.matrix.header.pivot_crumbs.get(id);
        //
        //The identified panel must be defined
        if (!panel)
            throw new schema.mutall_error(`No panel matches cell_id '${id}' among the header crumb panels in the pivot`);
        //
...
class footer extends panel {
    ...
    populate_cell(summary_name, data, column) {
        ...
        const td = this.matrix.get_cell(column, tr);
        //
...
class header_crumb extends draggable {
    ...
    get text() {
        ...
        const text = this.cell_id.get(this.factor_name);
        //
        if (text === undefined)
...
class filter extends draggable {
    ...
    //
    parent) {
        super({ region: 'crown', index }, { type: 'create', tagname: 'label', parent: parent.proxy }, parent);
        this.index = index;
        //
    ...
    create_select_element() {
        ...
        // same as item
        const select = this.create_element('select', this.proxy, { id });
        //
    ...
    update_value() {
        ...
        const paginator_index = this.sheet.crown.paginator.proxy.selectedIndex;
        //
