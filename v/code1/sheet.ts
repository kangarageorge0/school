...
//necessary. It is equivalent to this ordinary object definition
...
//Defin teh summary data for the footer summaries. It empty, all the psosiibe
//syummaryes    
...
class panel extends view {
    ...
    set_attributes(attr?: attributes) {
        ...
        if (attr.id !== undefined) this.set_id(attr.id)
    }
    ...
    set_id(id: string): void {
        ...
        //
        //Get its cell  index
        const cellIndex: number = this.element.cellIndex;
        //
...
class header extends panel {
    ...
    show_pivot_row(index: number): void {
        ...
        this.query.summaries.right.forEach(summary => new panel(
            type, //Element type
            this, //Parent
...
class body extends panel {
    ...
    public get_io_type(cell_id: cell_id, row: schema.fuel): io.io_type {
        //
    ...
    private populate_cell(id:string, value:lib.basic_value,tr:HTMLTableRowElement):void{
        ...
        //given one
        const td_bottom: HTMLTableCellElement = this.get_cell(id, tr);
        //
        //Get its column index
        const cellIndex:number = td_bottom.cellIndex;
        //
    ...
    public get_cell_properties(id:string, raw_value:string,data_row:fuel ):fuel|undefined{
        ...
        const cell_id:Map<factor_name, string> = this.get_cell_id(id);
        //
    ...
    //
    //
    get_cell_id(id:string):Map<factor_name, string>{
        //
        //
...
//    }
    ...
    //given one.
    get_cell(id: string, tr: HTMLTableRowElement): HTMLTableCellElement {
        //
        //Use the lookup to get the id
        const cellIndex: number | undefined = this.sheet.lookup.get(id);
        //
        //The id must yield a value; otherwise something went wrong
        if (cellIndex === undefined) throw new schema.mutall_error(`Id '${id}' is not found in the sheet lookup map`);
        //
        //Get the cell that matches the id
        const cell: HTMLTableCellElement = tr.cells[cellIndex];
        //
        return cell;
    }
}
...
class footer extends panel {
    ...
    populate_cell(summary_name: string, data: number | undefined, column: string) {
        //
        ...
        const tr: HTMLTableRowElement | null = this.get_tr(summary_name);
        //
        ...
        const td: HTMLTableCellElement | null = this.get_cell(tr, column);
        //
    ...
    get_tr(summary: string): HTMLTableRowElement | null {
        //
    ...
    //
    //Get the tr that matches the summary name
    get_cell(tr: HTMLTableRowElement | null, column: string): HTMLTableCellElement {
        //
        //Use the lookup to get the id
        const cellIndex: number | undefined = this.sheet.lookup.get(column);
        //
        //The id must yield a value; otherwise something went wrong
        if (cellIndex === undefined) throw new schema.mutall_error(`Id '${column}' is not found in the sheet lookup map`);
        //
        //Get the cell that matches the id(remember that the crest has been 
        //combined to one cell)
        const cell: HTMLTableCellElement = tr!.cells[cellIndex - this.factors.crest.length];
        //
        return cell;
    }
...
class header_crumb extends draggable {
    ...
    //
    //Redfine the parent panel to be a header
