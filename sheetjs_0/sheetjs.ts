...
type range = {
    ...
    //
    //The name to be associated with the range. It used for naming the
    //questionnaire matrix table -- the source of the data to be uploaded
    name:string,
    //
...
//Our local definition of a cell, used for reporting purposes, when we need to 
//pinpoint the source of an error
type cell = {address:xl.CellAddress, sheet:string}

//The excel class that provides all the excel services
export default class workbook extends view{
    //
    //Private storage for the current workbook
    private wb__?:xl.WorkBook;
    //
    //Public access to the private workbook storage
    set wb(w:xl.WorkBook){this.wb__=w};
    //
    //Throw an expetion if you try to access an unset workbook
    get wb(){
        if (!this.wb__) throw new mutall_error('Workbok not set');
        return this.wb__
    };
    //
    constructor(){
    ...
    async load_file():Promise<void>{
        ...
        const layouts:Array<layout> = await this.get_layouts(undefined);
        //
        //For this version., ignore the cell in our local layout.
        const layouts2:Array<quest.layout> = layouts.map(L=>L.layout);
        //
        console.log(layouts2);
        //
        //Load the data  to the database
        //const result:'Ok'|string = await exec('questionnaire', ['school'], 'load_common', [layouts2]);
        //
        //Report results
        //alert(result);
    }
    ...
    //
    //Wait to the user to select a workbook
    async get_workbook():Promise<xl.WorkBook>{
        //
        return new Promise(resolve=>{
            //
            //Get the files(s) to upload
            const files:FileList|null = (this.get_element('source') as HTMLInputElement).files;
            //
            //Discontinue if there is no file selected
            if (!files || files.length===0) {alert('Pease select a file'); return;}
            //
    ...
    create_workbook(evt:ProgressEvent<FileReader>, resolve:(wb:xl.WorkBook)=>void){
        //
    ...
    async get_layouts(source?: {type:'used_range'|'named_range', name:string}
        ...
        const range:range  = source ? this.get_range(source) : this.get_default_range();
        //
    ...
    get_range(source:{type:'used_range'|'named_range', name:string}):range{
        //
    ...
    get_named_range(name:string):range{
        ...
        const sheet_name:string = this.wb.SheetNames[index];
        //
    ...
    get_used_range(sheet_name:string):range{
        //
        //Get the desired  sheet from the inputs
        const sheet:xl.Sheet = this.wb.Sheets[sheet_name];
        //
    ...
    get_comments(name:xl.DefinedName):Array<layout>{
        ...
        const address:xl.CellAddress = xl.utils.decode_cell(name.Ref);
        //
        //Get our local version of the range from the reference
        const range:range = this.get_named_range(name.Name);
        //
    ...
    *collect_text_layouts(text:string, range?:range, cell?:cell):Generator<layout>{
        //
    ...
    //
    //[name, ...args]
    simplify_label(label:quest.label, range?:range, cell?:cell, i?:number):quest.label{
        //
        //
    ...
    simplify_dot_label(cell:cell, label:quest.label):quest.label{
        //
        ...
        const cell_address:string = xl.utils.encode_cell(cell.address);
        //
        //Get the addressed cell object
        const cellobj:xl.CellObject = this.wb.Sheets[cell.sheet][cell_address];  
        //
    ...
    simplify_below_label(cell:cell, label:quest.label, range:range):quest.label{
        //
    ...
    *collect_wb_labels(): Generator<layout>{
        //
        //Get the comment text
    ...
    *collect_cell_labels(local_range: range, body_start:number): Generator<layout>{
        ...
        for(let row=s.r; row<s.r+body_start; row++){
            ...
            for(let col=0; col<e.c; col++){
                ...
                const cellobj:xl.CellObject = this.wb.Sheets[local_range.sheet_name][addressA1];
                //
                ...
                const cell:cell = {address, sheet:local_range.name};
                //
                //Loop through the comments and collect the text layouts
                for(let comment of comments) yield *this.collect_text_layouts(comment.t, local_range, cell);
            }
        }    
    }
    ...
    collect_table_layout(range:range, body_start:number):layout{
        ...
        const cell:cell = {address:range.range.s, sheet:range.sheet_name};
        //
