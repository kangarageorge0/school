...
class crown extends panel {
    ...
    save_sheet_sql() {
        //
        if (this.query.all_ctes === undefined)
            throw new schema.mutall_error('Not (all) ctes sql is found');
        //
    ...
    async show() {
        ...
        this.filters = this.factors.crown.map((factor, index) => new filter(this, factor, index, this.legend));
        //
        //Show the paginator
    ...
    async get_paginator_data() {
        ...
        if (this.sheet.query.factors.crown.length === 0)
            return [];
        //
        ...
        //Create the (incomplete, i.e., false parameter) database, execute the 
        //sql to return the data
        [this.sheet.query.dbname, false], 'get_sql_data', [sql]);
        //
...
class draggable extends panel {
    ...
    constructor(tagname, parent, region, factor, index) {
...
class filter extends draggable {
    ...
    sync_with_paginator() {
        ...
        const paginator_index = this.sheet.crown.paginator.selectedIndex;
        //
        //Get the paginator data that matches the index. The paginator data
        //must be set by now
        const data = this.sheet.crown.paginator_data[paginator_index];
        //
