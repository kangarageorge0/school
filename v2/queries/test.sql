...
measurements as ( select 
                `school`, `year`, `class`, `exam`, `date`, `stream`, `student`, `subject`, 
 ...
 union all select 
            ...
            from grading),
crown as (select measurements.* from measurements  where school='KAPS' and year='2019' and class='7' and exam='DOUBLEMERIT EXAM' and date='2019-01-30'),

                crumb as (
                select
                    crown.*,
                    json_object(
                        'id', concat_ws('_', subject,measurement),
                        'value',value
                    ) as value2
                 from crown    
            ),
crest as (select stream,student, json_arrayagg(value2) as raw_values from crumb group by stream,student)
select * from crumb;