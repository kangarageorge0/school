...
//The structure of an Excel user response
type Iexcel =  
    //
    //Multiple files. Consider all selected files as workbook
    {
        type:'multiple',
        files:FileList;
    }
    //
    //Single workbook, refines to single sheet, single named range if applicable
    |{ 
        type:'single',
        //
        file:File,
        ...
        sheet_name?:string,
        //
        //...a named ramge in the given worksheet
        range_name?:string
    };
        


//Modelling a dirty (simple or compund) structure (excluding array). In general
//an input is made dirty, i.e., dirtified, by using a union to add the error
//data type as a possibility 
type dirty<x> = 
    //
    //The discriminant values, cannot be dirtified. The construct
    //{type:'single'|'multiple'} seems to be inconflict with the generalized
    //index, spelt out below. I dont undetstand why!
    x extends 'single'|'multiple' ? x 
    //
    //A dirty basic value or file list or file has errors added to it 
    //Include here all known compound structures that are exceptions to teh rule
    //below
    :x extends basic_value|FileList|File ?  x|Error  
    //
    //A dirty compound structure has noise added to the matching key. 
    :x extends {[key in keyof x]:x[key]} ? {[key in keyof x]:dirty<x[key]>}
    //
    //Any other structure, e.g., an array is reported as an error
    :never

//Let i be the promised data. Then resolve<i> is the promise, which resolves to i
type resolve<i> = (resolve:i)=>void;

//Types of requests (compiled from user inputs) for loading Excel data
type request =
    //
    //Load data from all the ranges (used or named) of all the sheets in the 
    //given workbooks
    {type:'workbooks', workbooks:Array<workbook>}
    //
    //Load the data from teh named range of the given workbook
    |{type:'named_range', workbook:workbook, range_name:string}
    //
    //Load data from all teh ranges (named or used) of the given sheet 
    |{type:'worksheet', workbook:workbook, sheet_name:string}
    //
    //Load all the data from all the shheets of the given  workbook
    |{type:'workbook', workbook:workbook}
    //
    //This situaion does not yoeld any layouts
    |undefined
...
export default class sheetjs extends view{
    ...
    async load_files():Promise<Iexcel|undefined>{
        //
        //Get the anchor tag (so that we can clean it later)
        const anchor:HTMLElement = this.get_element('anchor');
        //
        //Create a dialogbox by fetching from a html
        const dlg:HTMLDialogElement = await this.create_dialog(anchor);
        //
        //Wait for the user to save or abort the process
        const result:Iexcel|undefined = await new Promise(resolve=>{
            //
    ...
    //is in the server
    async test(workbook:string, sheet_name?:string, range_name?:string):Promise<'ok'|Error>{
        //
        //Convert the inputs to a request
        const request:request = await this.get_request_from_server(workbook, sheet_name, range_name);
        //
        //Execute the request
        //
        //Collect the layouts
        const layouts:Array<quest.layout> = [...this.collect_layouts(request)];
        //
        //Load the data  to the database
        const result:'Ok'|string = await exec('questionnaire', ['school'], 'load_common', [layouts]);
        //
    ...
    async create_dialog(anchor:HTMLElement):Promise<HTMLDialogElement>{
        //
        //Get the sheetjs html fragment
        const response:Response = await fetch('./sheetjs.html');
        //
        //Test that the response was ok
        if (!response.ok) throw new mutall_error(`Network error: ${response.statusText}`);
        //
        //Get the text string
       ...
       return <HTMLDialogElement>this.get_element('dialog');         
    }
    ...
    //error
    async collect_and_save_files(resolve:resolve<Iexcel|undefined>):Promise<void>{
        //
        ...
        const raw:dirty<Iexcel> = this.get_user_inputs();
        //
        //Collect the errors
        const errors:Array<{id:string, msg:string}> = [...this.collect_errors(raw)];
        //
        //If any, report them and do not continue.
        if (errors.length>0) {errors.forEach(e=>this.report_error(e.id, e.msg)); return}
        //
        //If the workbooks has no errors, then it is clean
        const clean =  <Iexcel> raw;
        //
        ...
        //data as the promise.
        if (result==='Ok') resolve(clean); else this.report_error('report', result);
    }
    ...
    async save_files(data:Iexcel):Promise<'Ok'|string>{
        //
    ...
    *collect_errors(data:dirty<Iexcel>):Generator<{id:string, msg:string}>{
        //
        switch(data.type){
            case "multiple":
                //
                ...
                if (files instanceof Error) yield {id:'workbooks', msg:files.message}
                break;
            case "single":
                //
                //Destructure the data.
                ...
                if (file instanceof Error) yield {id:'workbook', msg:file.message}
                if (sheet_name instanceof Error) yield {id:'sheet_name', msg:sheet_name.message}
                if (range_name instanceof Error) yield {id:'range_name', msg:range_name.message}
                //
                break;    
        }
    }
    ...
    */
    get_user_inputs():dirty<Iexcel>{
        ...
        //is flawed 
        const type = <'single'|'multiple'> this.get_value('type');
        //'
        if (type==='single'){
            //
            //Get the selected file 
            const input:FileList|null = (<HTMLInputElement>this.document.getElementById('single')).files;
            //
            //Return the only selected file, if any; otherwise an error
            const file:File|Error = input===null ? new Error('Select file'): input[0]; 
            //
            //Get the sheet name, converting null to undefined
            const sheet_name:string|Error| undefined= this.get_value('sheet_name') ?? undefined;
            //
            //Get the named range, converting null to undefined
            const range_name:string|Error|undefined = this.get_value('range_name')??undefined;
            // 
            return {type:'single', file, sheet_name, range_name}
        }else{
            //
            //Get the selected file 
            const input:FileList|null = (<HTMLInputElement>this.document.getElementById('multiple')).files;
            //
            //Return the only selected file, if any; otherwise an error
            const files:FileList|Error = input===null ? new Error('Select file'): input; 
            //
    ...
    //set of requests 
    async get_request_from_client(data:Iexcel):Promise<request>{
        //
        ...
        switch(data.type){
            ...
            case 'multiple':
                ...
                for(let file of Array.from(files)) result.push(await this.create_workbook(file))
                            //
            ...
            case 'single':
                ...
                if (range_name) return {type:'named_range', workbook, range_name}; 
                //
                //If a worksheet is defined, yield layouts from all her both used and named ranges
                if(sheet_name) return {type:'worksheet', workbook, sheet_name}
                //
    ...
    async get_request_from_server(file_name:string, sheet_name?:string, range_name?:string):Promise<request>{
        //
        //Convert file to a workbook
        const workbook:workbook = await this.get_workbook(file_name);
        //
        //If the range is defined, then the worksheet must be defined
        if (range_name){
            //
            if (!sheet_name) throw new mutall_error('The sheet name must accompany the range name');
            //
    ...
    async create_workbook(file:File):Promise<workbook>{
        //
    ...
    load_workbook(evt:ProgressEvent<FileReader>, resolve:(wb:workbook)=>void){
        //
        //Get the target, i.e., the file reader
        const target:FileReader|null= evt.target;
        //
        //If null, you have a problem
        if (!target) throw new mutall_error('You have an event handling problem!');
        //
        ...
        if (!data) throw new mutall_error('Null data content is not expected');
        //
        //Use teh data to create the workbbook
        const wb:xl.WorkBook = xl.read(data, { type: 'binary' });
        //
    ...
    *collect_layouts(request:request):Generator<quest.layout>{
        //
        ...
        switch(request.type){
            ...
            case "worksheet":
                ...
                const ws:worksheet|undefined = request.workbook.create_worksheet(request.sheet_name)
                //    
                if (ws) yield *ws.collect_layouts('complete');
                break;
            case "workbook":
                //
            ...
            case "workbooks":
                ...
                for(let wb of request.workbooks) yield *wb.collect_layouts('complete');       
        }
    }
...
abstract class comment extends view {
    //
    constructor(public native_workbook:xl.WorkBook){
        super();
    }

    //This methos should be implemented by all comment-base excel objects
    abstract collect_layouts(type:'complete'|'partial'):Generator<quest.layout>;
    ...
    *collect_layouts_from_text(
        text:string,
        workbook:workbook,
        worksheet?:worksheet, 
        range?:range, 
        address?:xl.CellAddress
    ):Generator<quest.label>{
        //
        ...
        for(let i=0; i<labels.length; i++) {
            //
            //Get the label
            const  label:quest.label = labels[i];
            //
    ...
    //
    //[name, ...args]
    simplify_label(label:quest.label, i?:number, cell?:xl.CellAddress, range?:range):quest.label{
        //
        //
        //Only cases where a defined cell and in a defined range can be simplified
        if (!cell || !range) return label;
        //
        ...
        if (!Array.isArray(label[0])) return label;
        //
    ...
    simplify_dot_label(cell:xl.CellAddress, label:quest.label, range:range):quest.label{
        //
        ...
        const cell_address:string = xl.utils.encode_cell(cell);
        //
        //Get the addressed cell object
        const cellobj:xl.CellObject = this.native_workbook.Sheets[range.sheet_name][cell_address];  
        //
        //Get the value of the cell object
        const value:basic_value|Date = cellobj?.v ?? null;
        //
        //Standardise the date value
        const new_value:basic_value = value instanceof Date ? view.standardise_date(value):value;
        //
    ...
    simplify_below_label(cell:xl.CellAddress, label:quest.label, range:range):quest.label{
        //
        ...
        const new_value:[string, ...any] = ['\\mutall\\capture\\lookup', range.name, col_index];
        //
...
export class workbook extends comment{
    ...
    public worksheets:Map<string, worksheet|undefined> = new Map();  
    //
    constructor(native:xl.WorkBook){
        super(native);
    }
    
    //
    //Collect workbook-level labels from comments in the workbook (file properties)
    *collect_layouts_from_properties(): Generator<quest.label>{
        //
        //Get the comment text
        const comment:string|undefined = this.native_workbook.Props?.Comments;
        //
    ...
    *collect_layouts(type:'complete'|'partial'):Generator<quest.layout>{
        //
        ...
        //uploading or not
        const all_worksheets:Array<worksheet|undefined>= this.native_workbook.SheetNames.map(sheet_name=>this.create_worksheet(sheet_name));
        //
        //Only the defined ines are considered
        const worksheets = <Array<worksheet>>all_worksheets.filter(ws=>ws instanceof worksheet);
        //
        //Collect layouts of each worksheet, partially, i.e., not as a complete
        //loadable units,  but as a part of a workbook loading set 
        for(let worksheet of worksheets) yield *worksheet.collect_layouts('partial');
    }

    //Create the named worksheet if necessary, i.e., if it is not in the
    //map
    create_worksheet(sheet_name:string):worksheet|undefined{
        //
        //Check if the worksheet exist; if it does, return it
        if (this.worksheets.has(sheet_name)) return this.worksheets.get(sheet_name);
        //
        //The sheet does not exist. Create it from first principles 
        //
        //Get the named sheet from this workbook
        const sheet:xl.Sheet =this.native_workbook.Sheets[sheet_name];
        //
        ...
        const ws:worksheet|undefined = ref===undefined 
            ? undefined 
...
abstract class range extends comment{
    //
    //There should be a worksheet associated with this range
    abstract get worksheet():worksheet; 
    //
    constructor(
        //
    ...
    //
    //The native range of this local version
    abstract get native():xl.Range;

    //Returns the name of a sheet associated with a range
    abstract get sheet_name():string;
    
    //Returns the name of a range
    ...
    *collect_layouts(type:'complete'|'partial'):Generator<quest.layout>{
        ...
        if (type==='complete') yield *this.worksheet.workbook.collect_layouts_from_properties();
        //
    ...
    *collect_layouts_from_header():Generator<quest.label>{
        //
        ...
        for(let row=s.r; row<s.r + this.body_start; row++)
            for(let col=0; col<e.c; col++) yield *this.collect_layouts_from_cell(row, col);
        
    }    

    //Collect layouts from cell comments
    *collect_layouts_from_cell(row:number, col:number):Generator<quest.label>{
        //
        ...
        const cellobj:xl.CellObject = this.native_workbook.Sheets[this.worksheet.sheet_name][addressA1];
        //
        //It looks like we can attach more than one comment in a cell!
        const comments:xl.Comments|undefined = cellobj?.c;
        //
        //Verify that indeed, comments exist
        if (!comments) return;
        //
        //Loop through the comments and collect the text layouts
        for(let comment of comments) yield *this.collect_layouts_from_text(comment.t, this.worksheet.workbook, this.worksheet, this, address);
    }
    
    //Rwturns the matrix of a range
    get_matrix():quest.matrix{
        //
        return {
            //
    ...
    get_table_body():Array<Array<basic_value>>{
        //
...
export class named_range extends range{
    //
    constructor(workbook:workbook, public name:string){
        super(workbook);
    }
    ...
    get worksheet():worksheet{
        //
        const ws:worksheet|undefined = this.workbook.create_worksheet(this.sheet_name);
        //
        //A named range must have a worksheet asociated witj it
        if (!ws) throw new mutall_error(`The range named ${this.name} in sheet ${this.sheet_name} has no worksheet`);
        //
        return ws;
    }
    ...
    get defined_name():xl.DefinedName{
        //
        //Get this workbook's properties. That is where names ranges are housed
        const props:xl.WBProps|undefined = this.native_workbook.Workbook;
        //
        //Check if there are any workbook properties
        if (!props) throw new Error(`No workbook properties found, so ${this.name} is not found`);
        //
        //Get defined names in the workbook
        const defined_names: Array<xl.DefinedName>|undefined = props.Names;
        if (!defined_names) throw new Error(`No names are defined, so ${this.name} cannot be found`);
        //
        //Check if the required names ias mong the defined ones
        const defined_name:xl.DefinedName|undefined= defined_names.find(dfn=>dfn.Name===this.name);
        //
        //Its an error if the named range does not exist
        if (!defined_name) throw new Error(`Range named ${this.name} is not found`);
        //
        return defined_name;
    }
    ...
    get sheet_name():string{
        ...
        const index:number|undefined = this.defined_name.Sheet
        //
        //If the index is not defined, then this is a globally scoped range
        //and should not be considerd for uploading
        if (!index) throw new mutall_error(`Worksheet number ${index} should is global and therefore not fit for uploading`);
        //
        //Get the sheet name at the requested index
        const sheet_name:string = this.native_workbook.SheetNames[index];
        //
        return sheet_name;
    }
    ...
    *collect_layouts(type:'complete'|'partial'):Generator<quest.layout>{
        ...
        //
        //Get the comment text
        const text:string|undefined = this.defined_name.Comment;
        //
        ...
        if (text) yield *this.collect_layouts_from_text(text, this.worksheet.workbook, this.worksheet, this);
        //
...
export class used_range extends range{
    ...
    get native():xl.Range {return this.worksheet.used_range};

    //The name of a used range is the same as that of the worksheet
    get name():string{return this.worksheet.sheet_name;}

    //Returns the native worksheet of this range; it is derived from the worksheet
    //used to construct the range
    get native_worksheet():xl.WorkSheet|undefined{
        //
        //Get the sheet name
        const sheet_name:string = this.worksheet.sheet_name;
        //
...
export class worksheet extends comment{
    ...
    constructor(public sheet_name:string, public workbook:workbook, public used_range:xl.Range){
        //
        super(workbook.native_workbook);
    }
    ...
    *collect_layouts(type:'complete'|'partial'):Generator<quest.layout>{
        ...
        if (type==='complete') yield *this.workbook.collect_layouts_from_properties(); 
        //
        //Collect all the named ranges in this worksheet.
        const nameds:Array<named_range> = this.collect_named_ranges();
        //
        //If there aren't  any, collect the (only) user range
        const ranges:Array<range> = nameds.length===0 ? [this.get_used_range()]: nameds;
        //
        //Use the ranges to collect tha layouts
        for(let range of ranges) yield *range.collect_layouts('partial');
    }

    //Return the used range (of this worksheet) if it is defined. The '!ref'
    //property of an xl.Worksheet refers to the used range.
    get_used_range():used_range{
        //
    ...
    collect_named_ranges():Array<named_range>{
        //
        //Get all the defined names in the current workbook
        const all_names:Array<xl.DefinedName>|undefined = this.native_workbook.Workbook?.Names;
        //
        //Return empty list if there are no defoned names
        if (!all_names) return [];
        //
        //Select those defined names that match with this sheet
        const dfnames:Array<xl.DefinedName> = all_names.filter(dfname=>this.is_matched(dfname))
        //
    ...
    is_matched(dfname:xl.DefinedName):boolean{
        //
        //Get the sheet's index from the defined name
        const index:number|undefined = dfname.Sheet;
        //
