...
function show_or_hide(member, dfname, status) {
    //
    ...
    const header = children.find(child => child.querySelector(`input[name ='${dfname}']`));
    // 
...
ref) {
    ...
    const members = children.filter(child => is_member(child, ref));
    //
    ...
    const selected = members.find(member => member.contains(ref));
    //
    //Separate the unselected members, i.e., those that were not selected
    const unselecteds = members.filter(m => m !== selected);
    // 
...
//Select only those children, i.e., members that are relevant for generating the output.
//Members are children that have a radio button named the same as the ref
function is_member(child, ref) {
    //
    //Get the name of the data field
    const dfname = ref.name;
    //
    //Look for an element within the child that has the same name
    const radios = Array.from(child.querySelectorAll(`input[type = "radio"][name ="${dfname}"]`));
    //
    const count = radios.length;
    //
    //Its a badly designed form if there is more than one element
    if (count > 1)
        throw `You have ${count} radio buttons in data field ${dfname} `;
    //
...
export async function append_form() {
    //
