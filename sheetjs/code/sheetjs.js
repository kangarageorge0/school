...
export class sheetjs extends view {
    ...
    async onload() {
        ...
        this.document
            ...
            .forEach(radio => radio.onclick = () => this.onclick(radio));
        //
    ...
    summarise_fields() {
        ...
        const list = this.document.querySelectorAll('[data-field]');
        //
        //Convert the nodelist to an array of field elements
        const fields = Array.from(list);
        //
    ...
    */
    ...
    onclick(ref) {
        ...
        const family_dna = `input[type="radio"][name="${ref.name}"]`;
        //
        //Define the dna for the elder child and its siblings in terms of the 
        //family's dna
        const dna = { siblings: family_dna, elder: family_dna + ":checked" };
        //
    ...
    get_siblings(mother, children, dna) {
        //
        ...
        const siblings = children.filter(child => this.contains(child, organs));
        //
        //Return the siblings
        return siblings;
    }
    //Tests if a child contains one and only one organ
    contains(child, organs) {
        //
    ...
    //
    //Populate the dropdown with worksheet names gotten from the workbook
    populate_worksheet(workbook) {
        //
        //Get the worksheet names to be populated to the selector
        const sheetnames = workbook.SheetNames;
        //
        //Get the selector
        const select = this.get_element('worksheet');
        //
        //Create the options
    ...
    //
    //Populate the dropdown with namedrange names gotten from the workbook
    populate_namedrange(workbook) {
        //
        //Get the worksheet selector and attach an onchange selector
        const worksheet_selector = this.get_element('worksheet');
        //
        //Get the worksheet names to be populated to the selector
        const sheetnames = workbook.Workbook?.Names;
        //
        //Get the selector
        const select = this.get_element('range');
        //
    ...
    populate_change(sheetname, worksheet_selector, select) {
        //
        //create options and set the value as the worksheet name
